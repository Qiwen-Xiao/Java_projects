...
public class Computer {
	...
	public int move(Human human, Random random) {
		int scoreOneRound = random.nextInt(6)+1;
		int scoreThisRound = 0;

		if(scoreOneRound != 6)
		{
			scoreThisRound += scoreOneRound;
			System.out.println("Computer's roll:" + scoreOneRound);
			while (scoreThisRound < 8 )
			{
				scoreOneRound = random.nextInt(6)+1;
				if(scoreOneRound != 6)
				{
					scoreThisRound += scoreOneRound;
					System.out.println("Computer's roll:" + scoreOneRound);
		...
		this.setScore(score + scoreThisRound);
